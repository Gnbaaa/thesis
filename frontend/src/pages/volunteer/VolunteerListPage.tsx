import { useMemo, useState } from 'react';
import { Link } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import { keepPreviousData, useQuery } from '@tanstack/react-query';
import { Badge } from '@/components/ui/Badge';
import { Button } from '@/components/ui/Button';
import { getAuthRole, useIsLoggedIn } from '@/lib/authSession';
import {
  catalogCard,
  catalogCardLink,
  catalogCardImage,
  catalogCardMedia,
  catalogCardMediaEmpty,
  catalogFocus,
  catalogPageBtn,
  catalogPageBtnActive,
  catalogPageShell,
  catalogPaginationRow,
  catalogSearchGrow,
  catalogSearchRow,
  catalogSelect,
} from '@/lib/catalogStyles';
import { cn } from '@/lib/cn';
import {
  listVolunteerPosts,
  type VolunteerDateRange,
  type VolunteerPostListResponse,
  type VolunteerPostStatus,
} from '@/features/volunteer/volunteerApi';

const pageSize = 8;

export default function VolunteerListPage() {
  const { t } = useTranslation();
  const loggedIn = useIsLoggedIn();
  const role = loggedIn ? getAuthRole() : null;
  const canCreate = role === 'ngo';

  const [q, setQ] = useState('');
  const [status, setStatus] = useState<'all' | VolunteerPostStatus>('all');
  const [range, setRange] = useState<'all' | VolunteerDateRange>('all');
  const [page, setPage] = useState(1);

  const query = useQuery<VolunteerPostListResponse>({
    queryKey: ['volunteer', { q, status, range, page }],
    queryFn: () =>
      listVolunteerPosts({
        q: q.trim() || undefined,
        status,
        range,
        page,
        pageSize,
      }),
    placeholderData: keepPreviousData,
  });

  const totalPages = useMemo(() => {
    const total = query.data?.total ?? 0;
    return Math.max(1, Math.ceil(total / pageSize));
  }, [query.data?.total]);

  return (
    <div className={catalogPageShell}>
        <div className={catalogSearchRow}>
          <input
            value={q}
            onChange={(e) => {
              setQ(e.target.value);
              setPage(1);
            }}
            placeholder={t('volunteer.searchPlaceholder')}
            className={cn(catalogSearchGrow, catalogFocus)}
            autoComplete="off"
          />
          {canCreate ? (
            <Link to="/volunteer/new" className="shrink-0">
              <Button size="sm" className="h-9 w-full sm:w-auto">
                {t('volunteer.addNew')}
              </Button>
            </Link>
          ) : null}
        </div>

        <div className="mt-4 flex flex-wrap gap-2">
          <select
            value={status}
            onChange={(e) => {
              setStatus(e.target.value as 'all' | VolunteerPostStatus);
              setPage(1);
            }}
            className={cn(catalogSelect, catalogFocus)}
          >
            <option value="all">{t('volunteer.filter.all')}</option>
            <option value="active">{t('volunteer.filter.active')}</option>
            <option value="completed">{t('volunteer.filter.completed')}</option>
          </select>
          <select
            value={range}
            onChange={(e) => {
              setRange(e.target.value as 'all' | VolunteerDateRange);
              setPage(1);
            }}
            className={cn(catalogSelect, catalogFocus)}
          >
            <option value="all">{t('volunteer.filter.dateAll')}</option>
            <option value="last7days">{t('volunteer.filter.last7days')}</option>
            <option value="last30days">{t('volunteer.filter.last30days')}</option>
          </select>
        </div>

        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">
          {query.isLoading ? (
            <p className="col-span-full text-sm text-text-muted">{t('common.loading')}</p>
          ) : query.data?.items?.length ? (
            query.data.items.map((post) => (
              <article key={post.id} className={catalogCard}>
                <div className={catalogCardMedia}>
                  {post.photoUrl ? (
                    <img src={post.photoUrl} alt="" className={catalogCardImage} loading="lazy" />
                  ) : (
                    <span className={catalogCardMediaEmpty}>{t('volunteer.card.imagePlaceholder')}</span>
                  )}
                </div>
                <div className="p-3">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="text-sm font-semibold text-text-heading">{post.title}</h3>
                    <Badge variant={post.status === 'active' ? 'success' : 'muted'}>
                      {post.status === 'active'
                        ? t('volunteer.status.active')
                        : t('volunteer.status.completed')}
                    </Badge>
                  </div>
                  <p className="mt-1 line-clamp-1 text-xs text-text-muted">{post.location}</p>
                  <p className="mt-0.5 text-xs text-text-muted">{post.eventDate}</p>
                  <Link to={`/volunteer/${post.id}`} className={cn(catalogCardLink, catalogFocus)}>
                    {t('volunteer.card.details')}
                  </Link>
                </div>
              </article>
            ))
          ) : (
            <p className="col-span-full text-sm text-text-muted">{t('volunteer.empty')}</p>
          )}
        </div>

        <div className={catalogPaginationRow}>
          <button
            type="button"
            className={cn(catalogPageBtn, catalogFocus)}
            onClick={() => setPage((p) => Math.max(1, p - 1))}
            disabled={page <= 1}
            aria-label={t('volunteer.pagination.prev')}
          >
            &lsaquo;
          </button>
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            .slice(0, 5)
            .map((p) => (
              <button
                key={p}
                type="button"
                className={cn(p === page ? catalogPageBtnActive : catalogPageBtn, catalogFocus)}
                onClick={() => setPage(p)}
              >
                {p}
              </button>
            ))}
          <button
            type="button"
            className={cn(catalogPageBtn, catalogFocus)}
            onClick={() => setPage((p) => Math.min(totalPages, p + 1))}
            disabled={page >= totalPages}
            aria-label={t('volunteer.pagination.next')}
          >
            &rsaquo;
          </button>
        </div>
    </div>
  );
}
